...
function direct_minmul(A::Matrix{T}, B::Matrix{T}, C::Matrix{T}, i::Int, j::Int) where{T}
    K = size(A)[2]
...
function check_all_minmul(A::Matrix{T}, B::Matrix{T}, C::Matrix{T}, D::Matrix{T}) where{T}
    M = size(A)[1]
    K = size(A)[2]
    N = size(B)[2]
...
@testset "Testing Matrix min mul" begin
    kernel = kernel_minmul()
    for (M, N, K) in [(0, 0, 0), (2, 0, 0), (2, 2, 0), (5, 6, 7), (101, 102, 103), (1024, 1024, 1024)]
        @testset "Float32 min mul" begin 
            ...
            @test check_all_minmul(A, B, C, D)
        end

        @testset "Float64 min mul" begin
