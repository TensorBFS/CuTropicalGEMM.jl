...
__global__ void Tropical_Gemm_kernel( 
    ...
    const int Thread_num_in_Warp_m = 4;
    const int Thread_num_in_Warp_n = 8;
    const int Warp_num_m = BLOCK_SIZE_M / (THREAD_SIZE_M * Thread_num_in_Warp_m);
    const int Warp_num_n = BLOCK_SIZE_N / (THREAD_SIZE_N * Thread_num_in_Warp_n);

    const int WarpId_in_Block_m = WarpId % Warp_num_m;
    const int WarpId_in_Block_n = WarpId / Warp_num_m;
    const int ThreadId_in_Warp_m = LaneId % Thread_num_in_Warp_m;
    const int ThreadId_in_Warp_n = LaneId / Thread_num_in_Warp_m;
    ...
    for (int tile_idx = 0 ; tile_idx < K ; tile_idx += BLOCK_SIZE_K) {
        ...
        for (int k = 0; k < BLOCK_SIZE_K; ++ k) {
            ...
            for (int thread_m = 0; thread_m < THREAD_SIZE_M; ++thread_m) {
                const int row = WarpId_in_Block_m * 4 * THREAD_SIZE_M + ThreadId_in_Warp_m * THREAD_SIZE_M + thread_m;
                A_reg[thread_m] = As[OFFSET(row, k, BLOCK_SIZE_M)];
            }

            #pragma unroll
            for (int thread_n = 0; thread_n < THREAD_SIZE_N; ++thread_n) {
                const int col = WarpId_in_Block_n * 8 * THREAD_SIZE_N + ThreadId_in_Warp_n * THREAD_SIZE_N + thread_n;
                B_reg[thread_n] = Bs[OFFSET(k, col, BLOCK_SIZE_K)];
            }
    ...
    for (int thread_n = 0; thread_n < THREAD_SIZE_N; ++thread_n) {
        #pragma unroll
        for (int thread_m = 0; thread_m < THREAD_SIZE_M; ++thread_m) {
            const int col = BLOCK_SIZE_N * blockIdx.y + WarpId_in_Block_n * 8 * THREAD_SIZE_N + ThreadId_in_Warp_n * THREAD_SIZE_N + thread_n;
            const int row = BLOCK_SIZE_M * blockIdx.x + WarpId_in_Block_m * 4 * THREAD_SIZE_M + ThreadId_in_Warp_m * THREAD_SIZE_M + thread_m;
            if (blockIdx.x == gridDim.x -1 || blockIdx.y == gridDim.y - 1) {
                if (row < M && col < N) {
