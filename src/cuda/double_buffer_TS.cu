...
__global__ void Tropical_Gemm_kernel( 
    ...
    float accum_add[THREAD_SIZE_MN] = {0};
    ...
    for (int tile_idx = 0 ; tile_idx < K ; tile_idx += BLOCK_SIZE_K) {
        ...
        for (int thread_m = 0; thread_m < THREAD_SIZE_M; ++thread_m) {
            #pragma unroll
            for (int thread_n = 0; thread_n < THREAD_SIZE_N; ++thread_n) {
                accum_add[OFFSET(thread_m, thread_n, THREAD_SIZE_M)] = OPERATOR_MUT(A_reg[thread_m], B_reg[thread_n]);
        ...
        #pragma unroll
        for (int k = 1; k < BLOCK_SIZE_K - 1; ++ k) {

            #pragma unroll
            for (int thread_m = 0; thread_m < THREAD_SIZE_M; ++thread_m) {
                #pragma unroll
                for (int thread_n = 0; thread_n < THREAD_SIZE_N; ++thread_n) {
                    accum[OFFSET(thread_m, thread_n, THREAD_SIZE_M)] = OPERATOR_ADD(accum[OFFSET(thread_m, thread_n, THREAD_SIZE_M)], accum_add[OFFSET(thread_m, thread_n, THREAD_SIZE_M)]);
                }
            }
