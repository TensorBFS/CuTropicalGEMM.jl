...
int main() {
    ...
    cudaMallocHost(&h_C, m * n * sizeof(int));
    cudaMallocHost(&h_D, m * n * sizeof(int));
    random_init(h_A, m * k);
    random_init(h_B, k * n);
    random_init(h_C, m * n);
    ...
    cudaMemcpy(d_A, h_A, m * k * sizeof(int), cudaMemcpyDefault);
    cudaMemcpy(d_B, h_B, k * n * sizeof(int), cudaMemcpyDefault);
    cudaMemcpy(d_C, h_C, m * n * sizeof(int), cudaMemcpyDefault);
    ...
    cudaMemcpy(h_D, d_C, m * n * sizeof(int), cudaMemcpyDefault);
...
// but in this script, we did not use any int4 type of some thing like that, it will be great to try to write a julia version of it
// extern "C"
// void SGemmMatMul(const int m, const int n, const int k, const int BM, const int BN, const int BK, const int TM, const int TN, int *d_A, int *d_B, int *d_C){
