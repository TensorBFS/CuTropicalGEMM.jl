...
// transfer long4
#define FETCH_long4(pointer) (reinterpret_cast<long4*>(&(pointer))[0])
...
int main() {
    ...
    const bool ENABLE_long_BUFFER = false;
    ...
    // warmup
    Tropical_Gemm_kernel<BLOCK_SIZE_M, BLOCK_SIZE_K, BLOCK_SIZE_N, THREAD_SIZE_M, THREAD_SIZE_N, ENABLE_long_BUFFER> 
    ...
    float tflops = (double(workload) / 1e12) / (double(ms) / 1e3);
...
// but in this script, we did not use any long4 type of some thing like that, it will be great to try to write a julia version of it
// extern "C"
// void SGemmMatMul(const int m, const int n, const int k, const int BM, const int BN, const int BK, const int TM, const int TN, long *d_A, long *d_B, long *d_C){
...
//     MatMul<BM, BK, BN, TM, TN, m, n, k, ENABLE_long_BUFFER> 
