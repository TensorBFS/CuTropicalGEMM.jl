...
void sgemm_128x128x8_kernel_maxadd(const float *A,
                            const float *B,
                            float *C,
                            uint32_t m,
                            uint32_t n,
...
extern "C"
void TropicalGemmMaxAdd(int m, int n, int k, float *d_A, float *d_B, float *d_C){

    dim3 grid((n + 127) / 128, (m + 127) / 128);
    sgemm_128x128x8_kernel_maxadd<<<grid, 256>>>(d_A, d_B, d_C, m, n, k, k * sizeof(float), n * sizeof(float) * 8);
