...
__global__ void kernel_TNN( 
    ...
    // registers for C
    float accum[THREAD_SIZE_M * THREAD_SIZE_N] = {PADDING};
    ...
    for (int tile_idx = 0 ; tile_idx < K ; tile_idx += BLOCK_SIZE_K) {
        ...
        for ( int i = 0 ; i < BLOCK_SIZE_N; i += B_TILE_COL_STRIDE) {
            ...
            if (blockIdx.x == gridDim.x -1 || tile_idx > K - BLOCK_SIZE_K) {
                Bs[OFFSET_row(B_TILE_ROW, i + B_TILE_COL, BLOCK_SIZE_N)] = row < K && col < N ? B[OFFSET_col(
                    row, // row
                    col, // col
                    K)] : PADDING;
            } else {
                Bs[OFFSET_row(B_TILE_ROW, i + B_TILE_COL, BLOCK_SIZE_N)] = B[OFFSET_col(
                    row, // row
                    col, // col
                    K)];
            }
        ...
        for (int k = 0; k < BLOCK_SIZE_K; ++ k) {
            ...
            for (int thread_y = 0; thread_y < THREAD_SIZE_M; ++thread_y) {
                #pragma unroll
                for (int thread_x = 0; thread_x < THREAD_SIZE_N; ++thread_x) {
                    accum[OFFSET_row(thread_y, thread_x, THREAD_SIZE_N)] = OPERATOR_ADD(OPERATOR_MUT(regs_a[thread_y], regs_b[thread_x]), accum[OFFSET_row(thread_y, thread_x, THREAD_SIZE_N)]);
                }
            }
...
bool check(const float *A,
    ...
    for (int i = 0; i < 100; ++i) {
        for (int j = 0; j < 100; ++j) {
            ...
            for (int p = 0; p < k; ++p) {
                sum = OPERATOR_ADD(sum, OPERATOR_MUT(A[OFFSET_row(a, p, k)], B[OFFSET_col(p, b, k)]));
            }
...
int main() {
    ...
    kernel_TNN<BLOCK_SIZE_M, BLOCK_SIZE_K, BLOCK_SIZE_N, THREAD_SIZE_M, THREAD_SIZE_N> 
    ...
    printf("FP32_maxplus_TN: %f TFLOPS\n", tflops);
