    ...
    for (int tile_idx = 0 ; tile_idx < K ;) {
        ...
        for (int k = 0; k < BLOCK_SIZE_K; k += 1) {
            ...
            for (int thread_n = 0; thread_n < THREAD_SIZE_N; ++thread_n) {
                B_reg[thread_n] = Bs[OFFSET_col(k, threadIdx.y * THREAD_SIZE_N + thread_n, BLOCK_SIZE_K)];
            }
...
bool check(const double *A,
    ...
    for (int i = 0; i < 100; ++i) {
        ...
        for (int j = 0; j < 100; ++j) {
            ...
            for (int p = 0; p < k; ++p) {
                sum = OPERATOR_ADD(OPERATOR_MUT(A[OFFSET_col(a, p, m)], B[OFFSET_col(p, b, k)]), sum);
            }
...
bool check_all(const double *A,
    ...
    for (int i = 0; i < m; ++i) {
        for (int j = 0; j < n; ++j) {
            ...
            for (int p = 0; p < k; ++p) {
                sum = OPERATOR_ADD(OPERATOR_MUT(A[OFFSET_col(i, p, m)], B[OFFSET_col(p, j, k)]), sum);
            }
...
int main() {
    ...
    printf("FP64_plusmul_NN: %f TFLOPS\n", tflops);
