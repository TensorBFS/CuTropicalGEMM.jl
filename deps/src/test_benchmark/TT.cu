...
bool check_all_TT(const float *A,
    const float *B,
    const float *C,
    const float *D,
    int m, int n, int k) {
    for (int a = 0; a < 100; ++a) {
        for (int b = 0; b < 100; ++b) {
            ...
            for (int p = 0; p < k; ++p) {
                sum = OPERATOR_ADD(OPERATOR_MUL(A[OFFSET_row(i, p, m, k)], B[OFFSET_row(p, j, k, n)]), sum);
            }
...
__global__ void CONCATENATETHREE(TYPENAME, FUNCNAME, TT)(TYPE * __restrict__ A, TYPE * __restrict__ B, TYPE * __restrict__ C, int M, int N, int K, TYPE alpha, TYPE beta) {
    ...
    for (int thread_m = 0; thread_m < THREAD_SIZE_M; ++thread_m) {
        #pragma unroll
        for (int thread_n = 0; thread_n < THREAD_SIZE_N; ++thread_n) {
            const int row = BLOCK_SIZE_M * blockIdx.y + THREAD_SIZE_M * threadIdx.y + thread_m;
            const int col = BLOCK_SIZE_N * blockIdx.x + THREAD_SIZE_N * threadIdx.x + thread_n;
...
int main() {
    ...
    CONCATENATETHREE(TYPENAME, FUNCNAME, TT)<BLOCK_SIZE_M, BLOCK_SIZE_K, BLOCK_SIZE_N, THREAD_SIZE_M, THREAD_SIZE_N> 
        <<< dimGrid, dimBlock >>>(d_A, d_B, d_C, m, n, k, alpha, beta);

    cudaMemcpy(h_D, d_C, m * n * sizeof(float), cudaMemcpyDefault);
    bool chk = check_all_TT(h_A, h_B, h_C, h_D, m, n, k);
    if (!chk) {
    ...
    printf("FP32_maxplus_TT: %f TFLOPS\n", tflops);
